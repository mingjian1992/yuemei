...
//		}
	...
	public static PageBean<v_supervisors> queryCustomers(int currPage, int pageSize,
		...
		try {
			count = (int) v_supervisors.count(condition.toString(), params.toArray());
			page = v_supervisors.find(condition.toString(), params.toArray()).fetch(currPage, pageSize);
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
	...
	public static List<v_supervisors> querySupervisors(List<Long> supervisorIds, ErrorInfo error) {
		...
		try {
			supervisors = v_supervisors.find(sql.toString(), params.toArray()).fetch();
		} catch (Exception e) {
			Logger.error(e.getMessage());
			e.printStackTrace();
			error.code = -1;
	...
	public static v_supervisors detail(long supervisorId, ErrorInfo error) {
		...
		try {
			supervisor = v_supervisors.findById(supervisorId);
		}catch(Exception e) {
			e.printStackTrace();
			Logger.info("数据库异常");
			error.code = -1;
